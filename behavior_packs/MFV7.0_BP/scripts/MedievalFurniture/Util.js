...
export function setOldDirection(e, value) {
	const {x, z} = e.player.getViewDirection();
	const isXDominant = Math.abs(x) > Math.abs(z);
	const directionValue = isXDominant ? (x > 0 ? value + 3 : value + 2) : (z > 0 ? value + 1 : value);
	e.permutationToPlace = e.permutationToPlace.withState('medieval:facing_direction', directionValue);
}
