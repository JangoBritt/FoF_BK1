...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('pog:cherryleavesDecay', {
        beforeOnPlayerPlace: e => {
            e.permutationToPlace = e.permutationToPlace.withState('pog:playerPlaced', true);
        },
        onRandomTick: e => {
            ...
            if (!foundLog) {
                if (e.block.permutation.getState('pog:playerPlaced') === false) {
                    e.block.setType('air');
                    e.block.dimension.runCommand(`loot spawn ${loc.x} ${loc.y} ${loc.z} loot "blocks/cherry_blossom_leaves_drops"`);
                }
            }
        }
    });
...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
    let tickCount = 0; // Initialize a counter for ticks
    const validLogBlocks = new Set(["minecraft:oak_log", "minecraft:birch_log", "minecraft:spruce_log", "minecraft:acacia_log", "minecraft:dark_oak_log", "minecraft:cherry_log", "minecraft:mangrove_log", "minecraft:jungle_log", "minecraft:pale_oak_log", "korbon:redwood_log", "korbon:baobab_log", "korbon:jacaranda_log", "korbon:redwood_wood", "korbon:baobab_wood", "korbon:jacaranda_wood"]);
    const radius = 5; // Change this value to adjust the radius (5x5x5=125) (4x4x4=64)
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('pog:redwoodleavesDecay', {
        beforeOnPlayerPlace: e => {
            e.permutationToPlace = e.permutationToPlace.withState('pog:playerPlaced', true);
        },
        onRandomTick: e => {
            ...
            if (!foundLog) {
                if (e.block.permutation.getState('pog:playerPlaced') === false) {
                    e.block.setType('air');
                    e.block.dimension.runCommand(`loot spawn ${loc.x} ${loc.y} ${loc.z} loot "blocks/redwood_leaves_drops"`);
                }
            }
        }
    });
